package com.codepandars.chemical.redisinterface.test.ltrim;
...
import com.codepandars.chemical.redisinterface.command.impl.ltrim.LTrimResponse;

public class LTrimCommandTest {
	@Test
	public void test() {
		
		LTrimRequest request = LTrimRequest.build("testltrim", "0", "-1") ;
		ICommand command = CommandFactory.getFactory(request).build(request) ;
		LTrimResponse response = command.execute();
