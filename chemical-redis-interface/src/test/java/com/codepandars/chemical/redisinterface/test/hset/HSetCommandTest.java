package com.codepandars.chemical.redisinterface.test.hset;
...
import com.codepandars.chemical.redisinterface.command.impl.hset.HSetResponse;

public class HSetCommandTest {
	@Test
	public void test() {
		
		HSetRequest hSetRequest = HSetRequest.build("test-hsetcommand", new Date().getTime()+"", new Date().toString()) ;
		ICommand command = CommandFactory.getFactory(hSetRequest).build(hSetRequest) ;
		HSetResponse response = command.execute() ;
		Assert.assertTrue(StringUtils.equals(response.getResult(),"1"));
//
